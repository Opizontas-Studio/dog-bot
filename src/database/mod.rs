...
impl BotDatabase {
    ...
    pub fn add_supervisor(&self, supervisor: Supervisor) -> Result<(), Box<dyn std::error::Error>> {
    ...
    pub fn get_active_supervisors(&self) -> Result<Vec<Supervisor>, Box<dyn std::error::Error>> {
        let read_txn = self.db.begin_read()?;
        let table = read_txn.open_table(SUPERVISORS_TABLE)?;
        ...
        for entry in iter {
            let (_, data) = entry?;
            let (supervisor, _): (Supervisor, _) =
                decode_from_slice(data.value(), bincode::config::standard())?;
    ...
    // Poll operations
    pub fn add_poll(&self, poll: Poll) -> Result<(), Box<dyn std::error::Error>> {
        let write_txn = self.db.begin_write()?;
        {
    ...
    pub fn update_poll(&self, poll: Poll) -> Result<(), Box<dyn std::error::Error>> {
        self.add_poll(poll) // Same as add since we're replacing
    }

    pub fn get_polls_by_stage(
        &self,
        stage: PollStage,
    ...
    pub fn delete_poll(&self, poll_id: u64) -> Result<(), Box<dyn std::error::Error>> {
        let write_txn = self.db.begin_write()?;
        {
            let mut table = write_txn.open_table(POLLS_TABLE)?;
    ...
    pub fn get_all_supervisors(&self) -> Result<Vec<Supervisor>, Box<dyn std::error::Error>> {
        let read_txn = self.db.begin_read()?;
        let table = read_txn.open_table(SUPERVISORS_TABLE)?;
    ...
    pub fn get_all_polls(&self) -> Result<Vec<Poll>, Box<dyn std::error::Error>> {
        let read_txn = self.db.begin_read()?;
        let table = read_txn.open_table(POLLS_TABLE)?;
        let mut polls = Vec::new();
